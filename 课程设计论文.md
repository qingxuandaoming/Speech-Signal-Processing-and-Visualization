...
注：在噪声较强或设备存在直流偏置的场景，可选进行去直流（`frame = frame - np.mean(frame)`），以提升鲁棒性；本项目运行时实现采用基于 `np.sign` 的过零统计，与上式等价但不做显式去直流。
...
- 在能量门控成立的前提下，系统还引入谱熵判据以增强噪声环境下的区分能力：当归一化谱熵 $\hat{H}(i) < T_H$ 时更倾向于语音（默认 `T_H=0.45`）。
- 综合决策采用 `(ZCR 判据) \lor (谱熵判据)` 作为能量门控后的次级判据，可兼顾清音、浊音与摩擦音。
- 引入攻击（attack）/保持（hold）/释放（release）平滑机制，降低阈值附近的抖动与误判粘连。
- 若开启自动阈值校准（`AUTO_CALIBRATE_THRESHOLDS=True`），系统在启动阶段采集前 `CALIBRATION_FRAMES=200` 帧估计噪声底，用于设定能量与 ZCR 的初始阈值；历史不足时回退到固定阈值策略。
